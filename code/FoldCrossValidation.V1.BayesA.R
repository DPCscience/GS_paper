FoldCrossValidation.V1.BayesA <- function(dataset, trait, genoID,M,nFolds, nRepeats){
  require(BGLR)  
  ...
  for (rep in 1:nRepeats){ 
    ...
    for (fold in 1:nFolds){
      ...
      fit.BayesA = BGLR(y=y,ETA=ETA.BayesA,nIter=10000,burnIn=1000,thin=5)
      pred.tst.BayesA=(as.matrix(M[ValidSet,])%*%as.matrix(fit.BayesA$ETA[[2]]$b))
      pred.tst.Bayes<-as.data.frame(pred.tst.BayesA)
      BLUPSthisFold<-data.frame(CLONE=rownames(pred.tst.BayesA), PRED=pred.tst.Bayes$V1)
