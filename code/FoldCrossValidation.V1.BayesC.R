# Fold cross-validation for BGLR crossvalidation bayesC--------------------------------------------------------------------
FoldCrossValidation.V1.BayesC <- function(dataset, trait, genoID,M,nFolds, nRepeats){
  require(BGLR)  
  ...
  for (rep in 1:nRepeats){ 
    ...
    for (fold in 1:nFolds){
      ...
      X = X/weight
      y = y/weight
      ETA.BayesC = list(list(X = X, model = "FIXED"), list(X = Z%*%M.trn, model = "BayesC"))
      fit.BayesC = BGLR(y=y,ETA=ETA.BayesC,nIter=10000,burnIn=1000,thin=5)
      pred.tst.BayesC=(as.matrix(M[ValidSet,])%*%as.matrix(fit.BayesC$ETA[[2]]$b))
      pred.tst.Bayes<-as.data.frame(pred.tst.BayesC)
      BLUPSthisFold<-data.frame(CLONE=rownames(pred.tst.BayesC), PRED=pred.tst.Bayes$V1)
